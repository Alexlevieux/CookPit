...
public class cookpitProvider extends ContentProvider {
    ...
    static final int DISH_WITH_DRAWABLE_ID = 204;
    ...
    static final int DRAWABLE = 1400;
    static final int DRAWABLE_WITH_DISH_ID = 1401;


    static final int FTS_MENU = 1500;
    static final int FTS_DISH = 1510;
    static final int FTS_MEP = 1520;
    static final int FTS_INGREDIENT = 1530;
    ...
    static UriMatcher buildUriMatcher() {
        ...
        matcher.addURI(authority, KitchenContract.PATH_DRAWABLE, DRAWABLE);
        matcher.addURI(authority, KitchenContract.PATH_DRAWABLE + "/#", DRAWABLE_WITH_DISH_ID);
        return matcher;
    }
        ...
        switch (match) {
            ...
            case DRAWABLE: {
                String[] projection = new String[]{
                        KitchenContract.DrawableEntry._ID,
                ...
                String DrawableFileName = values.getAsString(KitchenContract.DrawableEntry.COLUMN_FILE_NAME);

                String filenameColumn = KitchenContract.DrawableEntry.COLUMN_FILE_NAME + "=?";
                String[] fileNameArg = new String[]{DrawableFileName};
                int nbrOfRow = 0;
                try {
                    Cursor c = db.query(KitchenContract.DrawableEntry.TABLE_NAME, projection, filenameColumn, fileNameArg, "", "", "");
                    if (c.getCount() > 0) {
                        c.moveToFirst();

                        int id = c.getInt(c.getColumnIndex(KitchenContract.DrawableEntry._ID));
                        values.put(KitchenContract.DrawableEntry._ID, id);
                        Log.v("file already exist", "pasGrave");
                        nbrOfRow = db.update(KitchenContract.DrawableEntry.TABLE_NAME, values, KitchenContract.DrawableEntry._ID + " = " + values.getAsString(KitchenContract.DrawableEntry._ID), null);


                    } else {
                        long _id = db.insert(KitchenContract.DrawableEntry.TABLE_NAME, null, values);

                        if (_id > 0)
                            returnUri = KitchenContract.DrawableEntry.BuildDrawableUri(_id);
                        else {
                            nbrOfRow = db.update(KitchenContract.DrawableEntry.TABLE_NAME, values, KitchenContract.DrawableEntry._ID + " = " + values.getAsString(KitchenContract.DrawableEntry._ID), null);
                        }
                    }
                    if (nbrOfRow > 0) {
                        returnUri = KitchenContract.DrawableEntry.BuildDrawableUri(values.getAsLong(KitchenContract.DrawableEntry._ID));
                    }
                    if (returnUri == null) {
                        throw new android.database.SQLException("Failed to insert row into " + uri);
                    }
